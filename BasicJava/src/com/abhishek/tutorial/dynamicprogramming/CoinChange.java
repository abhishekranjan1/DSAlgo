...
public class CoinChange {
    ...
    private static int coinChangeMinimum(int[] coins, int amount) {
        ...
        for(int coin=0; coin<coins.length;coin++){
            for(int amt=coins[coin]; amt <= amount; amt++){
                if(coins[coin] <= amt)
                    dp[amt] = Math.min(dp[amt],1+dp[amt-coins[coin]]);
            }
            System.out.print("*"+coins[coin]+"*"+"("+coin+")==");
            dpPrintAfterCoinsSelection(dp);
            System.out.println();
        }
    ...
    private static void dpPrintAfterCoinsSelection(int[] dp) {
        int i=0;
        for(; i< dp.length-1;i++){
